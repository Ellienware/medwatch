...
async function encryptTestResultData(data) {
...
async function decryptTestResultData(data, fieldsToDecrypt) {
...
function hasPermission(role, action, isSensitive = false) {
    const permissions = {
...
function getAllowedFields(role, isSensitive = false) {
    const fieldAccess = {
        super_admin: ["*"],
        clinic_admin: ["*"],
    ...
    try {
        ...
        switch (payload.action) {
            case "create": {
                ...
                // Create test result
                const testResult = await databases.createDocument(process.env.APPWRITE_DATABASE_ID, "test_results", node_appwrite_1.ID.unique(), encryptedData);
                // Audit log
            ...
            case "read": {
                ...
                try {
                    // Get test result
                    const testResult = await databases.getDocument(process.env.APPWRITE_DATABASE_ID, "test_results", payload.testResultId);
                    // Check clinic access
                    ...
                    if (testResult.test_code) {
                        try {
                            const test = await databases.getDocument(process.env.APPWRITE_DATABASE_ID, "clinical_tests", testResult.test_code);
            ...
            case "update": {
                ...
                try {
                    ...
                    const existingTestResult = await databases.getDocument(process.env.APPWRITE_DATABASE_ID, "test_results", payload.testResultId);
                    // Check clinic access
                    ...
                    // Update test result
                    const updatedTestResult = await databases.updateDocument(process.env.APPWRITE_DATABASE_ID, "test_results", payload.testResultId, encryptedUpdates);
            ...
            case "review": {
                ...
                try {
                    ...
                    // Update test result
                    const updatedTestResult = await databases.updateDocument(process.env.APPWRITE_DATABASE_ID, "test_results", payload.testResultId, reviewData);
                    // Decrypt for response
                    const allowedFields = getAllowedFields(payload.userRole, existingTestResult.is_sensitive);
                    const fieldsToDecrypt = allowedFields.includes("*")
                        ? ["findings", "recommendations", "notes", "results", "technician_notes", "review_notes"]
            ...
            case "list": {
                ...
                const queries = [
                    ...
                    node_appwrite_1.Query.limit(payload.filters?.limit || 100),
                    node_appwrite_1.Query.orderDesc("performed_at")
                ];
                // Apply filters
                if (payload.filters?.appointment_id) {
                    queries.push(node_appwrite_1.Query.equal("appointment_id", payload.filters.appointment_id));
                }
                if (payload.filters?.patient_id) {
                    queries.push(node_appwrite_1.Query.equal("patient_id", payload.filters.patient_id));
                }
                if (payload.filters?.status) {
                    queries.push(node_appwrite_1.Query.equal("status", payload.filters.status));
                }
                if (payload.filters?.requires_review) {
                    queries.push(node_appwrite_1.Query.equal("requires_review", true));
                }
                if (payload.filters?.test_code) {
                    queries.push(node_appwrite_1.Query.equal("test_code", payload.filters.test_code));
                }
                // Get test results
                const response = await databases.listDocuments(process.env.APPWRITE_DATABASE_ID, "test_results", queries);
                // Decrypt each test result based on role
                const decryptedResults = await Promise.all(response.documents.map(async (doc) => {
                ...
                if (payload.filters?.includeStats) {
                    ...
                    const sensitiveCount = response.documents.filter((d) => d.is_sensitive).length;
                    const pendingReview = response.documents.filter((d) => d.requires_review && !d.reviewed_by).length;
                    stats = {
                        total,
                        sensitiveCount,
                        pendingReview,
                        normalCount: response.documents.filter((d) => d.is_normal === true).length,
                        abnormalCount: response.documents.filter((d) => d.is_normal === false).length,
                    };
                }
                result = {
                    success: true,
            ...
            case "delete": {
                ...
                try {
                    ...
                    // Delete test result
                    await databases.deleteDocument(process.env.APPWRITE_DATABASE_ID, "test_results", payload.testResultId);
                    // Audit log
                    await createAuditLog(databases, {
                        clinic_id: payload.clinicId,
                        user_id: payload.userId,
