...
export class ClinicalTestRepository extends BaseRepository<ClinicalTest> {
  ...
  async findByClinicId(clinicId: string, options?: { isActive?: boolean; category?: string }): Promise<ClinicalTest[]> {
  ...
  async findByTestCode(testCode: string, clinicId: string): Promise<ClinicalTest | null> {
    const tests = await this.find([Query.equal("test_code", testCode), Query.equal("clinic_id", clinicId)])
    return tests[0] || null
  }
  ...
  async findByCategory(clinicId: string, category: string): Promise<ClinicalTest[]> {
    return this.find([
  ...
  async getCategories(clinicId: string): Promise<string[]> {
    ...
    const categories = new Set(tests.map((test) => test.test_category).filter((cat): cat is string => !!cat))
